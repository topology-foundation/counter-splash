  ...
  function getRandomShape(
    ...
    do {
      position = [
        ...
        seedRandom(seed + 2) ** 2 * containerHeight +
          center[1] -
          containerHeight / 2, // random y position within the box
        seedRandom(seed + 3) * containerLength +
          center[2] -
          containerLength / 2, // random z position within the box
      ];
...
{#each randomShapes as { shape, size, position, color, rotation }}
  ...
  {:else}
    <AutoColliders shape={meshShape[shape]} friction={0} restitution={0}>
      <T.Mesh {position} {rotation} name={shape}>
        {#if shape === "cylinder"}
