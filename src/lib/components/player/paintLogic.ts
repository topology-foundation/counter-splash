...
export function handlePainting() {
  if (get(isMouseDown) && get(paintMode) && !get(isIntersect)) {
    const x = Math.floor(get(mousePosition).x * 4000);
    const y = Math.floor(get(mousePosition).y * 3000);
    ...
    lastMousePosition = { x: get(mousePosition).x, y: get(mousePosition).y };
  } else {
